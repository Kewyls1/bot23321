                    ...
                    if (!error) {
                        ...
                        message.author.send(
                            new MessageEmbed()
                                ...
                                .addField('Serwis', `\`\`\`${args[0][0].toUpperCase()}${args[0].slice(1).toLowerCase()}\`\`\``, true)
                                .addField('Konto', `\`\`\`${firstLine}\`\`\``, true)
                                .setTimestamp()
                        )
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function(error) {
                                message.channel.send(
                                    new MessageEmbed()
                                        .setColor(config.color.green)
                                        .setTitle('Konto wygenerowano pomyślnie!')
                                        .setDescription(`Sprawdź prywatne wiadomości ${message.author}! *Jeśli nie otrzymałeś wiadomości, odblokuj prywatne wiadomości!*`)
                                        .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                        .setTimestamp()
                                );
                            ...
                            return message.channel.send(
                                new MessageEmbed()
                                    .setColor(config.color.red)
                                    .setTitle('Błąd!')
                                    .setDescription(`Wyczerpano zapas kont: \`${args[0]}\``)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                            );
                        };
                        ...
                        return message.channel.send(
                            new MessageEmbed()
                                .setColor(config.color.red)
                                .setTitle('Błąd!')
                                .setDescription(`Nie ma takiego generatora! **Sprawdź poprawność liter oraz wielkość**`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                        );
                    };
